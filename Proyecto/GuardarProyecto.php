...
if ($_POST['acc'] == "1") {
    ...
    if (isset($_POST["Dat_Financiacion"])) {
        foreach ($_POST["Dat_Financiacion"] as $key => $val) {
            ...
            $consulta2 = "INSERT INTO banco_proyec_financiacion VALUES(null,'" . $id_Proy . "','" . $parFinan[0] . "'," . $parFinan[1] . ",'" . $parFinan[2] . "')";
            // echo $consulta2;
            $qc2 = mysqli_query($link, $consulta2);
            if (($qc2 == false) || (mysqli_affected_rows($link) == -1) || mysqli_errno($link) != 0) {
                $success = 0;
    ...
    if (isset($_POST["Dat_Presupuesto"])) {
        foreach ($_POST["Dat_Presupuesto"] as $key => $val) {
            ...
            $consulta2 = "INSERT INTO banco_proyec_presupuesto VALUES(null,'" . $id_Proy . "','" . $parPres[0] . "','" . $parPres[1] . "','" . $parPres[2] . "'," . $parPres[3] . "," . $parPres[4] . ")";
            // echo $consulta2;
            $qc2 = mysqli_query($link, $consulta2);
            if (($qc2 == false) || (mysqli_affected_rows($link) == -1) || mysqli_errno($link) != 0) {
                $success = 0;
