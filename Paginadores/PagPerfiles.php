...
if ($busq != "") {
    ...
    $consulta = "SELECT * FROM ".$_SESSION['ses_BDBase'].".perfiles WHERE  ";
    ...
    $consulta .= " order by nomperfil ASC LIMIT " . $regemp . "," . $regmos;
} else {

    $consulta = "SELECT * FROM ".$_SESSION['ses_BDBase'].".perfiles "
            . " order by nomperfil ASC LIMIT " . $regemp . "," . $regmos;
}
...
if (mysqli_num_rows($resultado) > 0) {
    ...
    while ($fila = mysqli_fetch_array($resultado)) {
        ...
        $cad .= "<tr>"
                ...
                . "<table>"
                . "<tr>";
        if ($_SESSION['GesUsuMPe'] == "s") {
            $cad .= "<td>"
                    . "<a  onclick=\"$.editPerf('" . $cod . "')\"  class=\"btn default btn-xs blue\">"
                    . "<i class=\"fa fa-edit\"></i> Editar</a>"
                    . "</td>";
        }
        $cad .= "<td>"
                . "<a   onclick=\"$.verPerf('" . $cod . "')\"  class=\"btn default btn-xs blue\">"
                . "<i class=\"fa fa-search\"></i> Ver</a>"
                . "</td>";
        if ($_SESSION['GesUsuEPe'] == "s") {
            $cad .= "<td>"
                    . "<a onclick=\"$.deletPerf('" . $fila["nomperfil"] . '/' . $cod . "')\" class=\"btn default btn-xs red\">"
...
$consulta = "SELECT count(*) as conta FROM ".$_SESSION['ses_BDBase'].".perfiles";
...
if ($contador > $regmos) {
    ...
    if ($pagact > 1) {
        ...
        $cad2 = $cad2 . "<input type='button' style=\"width: 70px;\" class=\"btn btn-circle blue btn-sm dropdown-toggle\" value=' << ' onclick=\"$.paginador('1','../paginador_centros');\" disabled style='padding: 4px 4px 4px 4px' /></td>";
    ...
    if ($pagact < $div) {
        ...
        $cad2 = $cad2 . "<td><input type='button' style=\"width: 70px;\" class=\"btn btn-circle blue btn-sm dropdown-toggle\" value=' >> ' onclick=\"$.paginador('" . $div . "','../paginador_centros');\" disabled style='padding: 4px 4px 4px 4px' /><input type='hidden' id='txttotal' value='" . $div . "' />"
                . "<input type='hidden' id='codter' name='codter' value='' /></td>";
    }
