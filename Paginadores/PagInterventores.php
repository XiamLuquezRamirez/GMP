...
if ($busq != "") {
    ...
    $consulta = "SELECT * FROM interventores WHERE ";
    ...
    $consulta .= " AND estado_interventores='ACTIVO' order by nom_interventores ASC LIMIT " . $regemp . "," . $regmos;
} else {

    $consulta = "SELECT * FROM interventores WHERE estado_interventores='ACTIVO' order by nom_interventores ASC  LIMIT " . $regemp . "," . $regmos;
}
...
if (mysqli_num_rows($resultado) > 0) {
    ...
    while ($fila = mysqli_fetch_array($resultado)) {
        ...
        $cod = $fila["id_interventores"];
        $cad .= "<tr>"
                . "<td class=\"highlight\">"
                . $fila["cod_interventores"] . " "
                . "</td>"
                . "<td class=\"highlight\">"
                . $fila["nom_interventores"] . ""
                . "</td>"
                . "<td class=\"highlight\">"
                . $fila["correo_interventores"] . ""
                . "</td>"
                . "<td class=\"highlight\">"
                . "<table>"
                . "<tr>";
        if ($_SESSION['GesParMInt'] == "s") {
            $cad .= "<td>"
                    . "<a  onclick=\"$.editInterv('" . $cod . "')\"  class=\"btn default btn-xs purple\">" .
                    "<i class='fa fa-edit'></i> Editar "
                    . "</a>"
                    . "</td>";
        }
        $cad .= "<td>"
                . "<a onclick=\"$.VerInterv('" . $cod . "')\" class='btn default btn-xs blue'>" .
                "<i class='fa fa-search'></i> Ver "
                . "</a>"
                . "</td>";
        if ($_SESSION['GesParEInt'] == "s") {
            $cad .= "<td>"
                    . "<a onclick=\"$.deletInterv('" . $cod . "')\" class='btn default btn-xs red'>" .
...
$consulta = "SELECT count(*) as conta FROM interventores WHERE estado_interventores='ACTIVO' ";
