...
if ($busq != "") {
    ...
    $buscar = explode(" ", $busq);
    $flag = "s";
    $consulta = "SELECT * FROM  dependencias  WHERE ";
    ...
    $consulta .= " AND estado='ACTIVO' order by des_dependencia ASC LIMIT " . $regemp . "," . $regmos;
} else {

    $consulta = "SELECT * FROM dependencias WHERE estado='ACTIVO' order by des_dependencia ASC  LIMIT " . $regemp . "," . $regmos;
}
...
if (mysqli_num_rows($resultado) > 0) {
    ...
    while ($fila = mysqli_fetch_array($resultado)) {
        $contador++;
        $cod = $fila["iddependencias"];
        $cad .= "<tr>"
                . "<td class=\"highlight\">"
                . $fila["cod_dependencia"] . " "
                . "</td>"
                . "<td class=\"highlight\">"
                . $fila["des_dependencia"] . ""
                . "</td>"
                . "<td class=\"highlight\">"
...
if ($flag == "n") {
    $consulta = "SELECT count(*) as conta FROM dependencias WHERE estado='ACTIVO' ";
