...
$cad = "<table id='testTable' class=\"table table-bordered table-striped table-hover table-condensed flip-content\" role=\"grid\" >"
        . "<thead>"
        . "<tr>"
        . "<th>"
...
if ($busq != "") {
    ...
    $consulta = "SELECT * FROM ejes ej left join dimensiones dim on ej.DIMENSION=dim.id WHERE ";
    ...
    $consulta .= " AND ej.ESTADO='ACTIVO' order by NOMBRE ASC LIMIT " . $regemp . "," . $regmos;
} else {

    $consulta = "SELECT * FROM ejes ej left join dimensiones dim on ej.DIMENSION=dim.id  WHERE ej.ESTADO='ACTIVO' order by NOMBRE ASC  LIMIT " . $regemp . "," . $regmos;
}
...
if (mysqli_num_rows($resultado) > 0) {
    while ($fila = mysqli_fetch_array($resultado)) {
        ...
        $cad .= "<a   onclick=\"$.VerEjes('" . $cod . "')\"  class=\"btn default btn-xs blue\">"
                . "<i class=\"fa fa-search\"></i> Ver</a>";
        if ($_SESSION['GesPlaEEj'] == "s") {
            $cad .= "<a onclick=\"$.deletEjes('" . $cod . "')\" class=\"btn default btn-xs red\">"
