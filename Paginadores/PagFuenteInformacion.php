...
if ($busq != "") {
    ...
    $buscar = explode(" ", $busq);
    $flag = "s";
    $consulta = "SELECT * FROM  fuente_informacion  WHERE ";
    ...
    $consulta .= " AND estado='ACTIVO' order by id ASC LIMIT " . $regemp . "," . $regmos;
} else {

    $consulta = "SELECT * FROM fuente_informacion WHERE estado='ACTIVO' order by id ASC  LIMIT " . $regemp . "," . $regmos;
}
...
if (mysqli_num_rows($resultado) > 0) {
    ...
    while ($fila = mysqli_fetch_array($resultado)) {
        ...
        if ($_SESSION['GesParMDep'] == "s") {
            $cad .= "<a  onclick=\"$.editFuente('" . $cod . "')\"  class=\"btn default btn-xs purple\">" .
                    "<i class='fa fa-edit'></i> Editar "
                    . "</a>";
        }
        $cad .= "<a onclick=\"$.VerFuente('" . $cod . "')\" class='btn default btn-xs blue'>" .
                "<i class='fa fa-search'></i> Ver "
                . "</a>";
        if ($_SESSION['GesParEDep'] == "s") {
            $cad .= "<a onclick=\"$.deletFuente('" . $cod . "')\" class='btn default btn-xs red'>" .
...
if ($flag == "n") {
    $consulta = "SELECT count(*) as conta FROM fuente_informacion WHERE estado='ACTIVO' ";
