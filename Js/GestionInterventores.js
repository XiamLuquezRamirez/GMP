$(document).ready(function() {
    ...
    $("#menu_op_intervent").addClass("active");
    var Op_Validar = [];
    var Op_Vali = "Ok";
    ...
    $.extend({
        Interv: function() {
            ...
            $.ajax({
                type: "POST",
                url: "../Paginadores/PagInterventores.php",
                data: datos,
                dataType: 'JSON',
                success: function(data) {
                    $('#tab_Interv').html(data['cad']);
                    $('#bot_Interv').html(data['cad2']);
        ...
        editInterv: function(cod) {
            ...
            $.ajax({
                ...
                dataType: 'JSON',
                success: function(data) {
                    $('#txt_Cod').val(data['cod_interventores']);
                    $('#txt_Desc').val(data['nom_interventores']);
                    $('#txt_Corre').val(data['correo_interventores']);
                    $('#txt_Telf').val(data['telef_interventores']);
                    $('#txt_obser').val(data['obser_interventores']);
                    $('#txt_id').val(cod);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data === "bien") {
                            ...
                            $.Alert("#msg2", "Este Interventor no se puede Eliminar, Se encuentra relacionado a un Contrato", "warning", "warning");
                        }
                    },
                    error: function(error_messages) {
                        alert('HA OCURRIDO UN ERROR');
