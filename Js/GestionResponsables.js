$(document).ready(function() {
    ...
    $("#menu_op_responsa").addClass("active");
    var Op_Validar = [];
    var Op_Vali = "Ok";
    ...
    $.extend({
        respon: function() {
            ...
            $.ajax({
                type: "POST",
                url: "../Paginadores/PagResponsables.php",
                data: datos,
                dataType: 'JSON',
                success: function(data) {
                    $('#tab_Responsables').html(data['cad']);
                    $('#bot_Responsables').html(data['cad2']);
        ...
        editResponsa: function(cod) {
            ...
            $.ajax({
                ...
                dataType: 'JSON',
                success: function(data) {
                    $('#txt_Cod').val(data['cod_responsable']);
                    $('#txt_Desc').val(data['nom_responsable']);
                    $('#txt_Corre').val(data['email_responsable']);
                    $('#txt_Telf').val(data['tel_responsable']);
                    $('#txt_obser').val(data['obs_responsable']);
                    $('#CbDependencia').val(data["dependencia"]).change();
                    $('#CbUsuarios').val(data["usuario"]).change();
                    $('#txt_id').val(cod);
        ...
        deletResponsa: function(cod) {
            if (confirm("\xbfEsta seguro de realizar la operaci\xf3n?")) {
                ...
                $.ajax({
                    type: "POST",
                    url: "../Administracion/GuardarResponsable.php",
                    data: datos,
                    success: function(data) {
                        if (data === "bien") {
        ...
        Validar: function() {
            ...
            if (Value === "" || Value === " ") {
                Op_Validar.push("Fail");
                $("#From_Dependencia").addClass("has-error");
            } else {
                $("#From_Dependencia").removeClass("has-error");
                Op_Validar.push("Ok");
            }
